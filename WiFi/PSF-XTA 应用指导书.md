...
             本手册介绍了PSF-XTA应用指导书产品参数，包含以下章节。 

| 章     | **标题**           | **内容**                           |
| :----- | :----------------- | :--------------------------------- |
| 第一章 | 产品简介           | 概述PSF-XTA的特点和功能应用        |
...
               PSF-XTA是深圳酷宅科技有限公司（简称：酷宅科技）一款基于ESP8285的超低功耗通用串口转Wi-Fi透传模

块，可以用于快速开发基于Wi-Fi的数据传输产品。PSF-XTA系列模块能实现Wi-Fi的搜索连接，网络服务器的通

讯，串口转Wi-Fi透传，手机APP的控制等基本功能，根据具体型号不同支持不同地区的服务器及各种手机端控

制。

        PSF-XTA封装方式多样，天线可支持IPEX接口天线、陶瓷天线。可广泛应用于智能电网、智能交通、智能家
...
|  天线类型  | PSF-ATA：IPEX接口天线<br>PSF-BTA：陶瓷贴片天线               |
...
            PSF-XTA多通道开关模块包含两种型号：PSF-ATA、PSF-BTA。两种模块的区别只是天线类型的不同，PSF-

ATA使用IPEX接口天线，PSF-BTA使用陶瓷天线，其他方面没有区别。
...
| 19   | GPIO5  | NC                                                           |
| 20   | GND    | GND                                                          |
| 21   | RX     | 串口输入、波特率可调且仅支持19200（1位起始位，8位数据位，1位停止位，无校验位） |
| 22   | TX     | 串口输出、波特率可调且仅支持19200（1位起始位，8位数据位，1位停止位，无校验位） |
| 23   | 3V3    | 电源                                                         |
...
            PSF-XTA模块可作为一个通用串口转Wi-Fi透传模块，将外部串行通信的简单设备实现在网络上的通信。以

下是功能说明：

       1、配置方式：快速配对模式/兼容配对模式。长按按键（IO0下拉，默认高电平）5s以上进入快速配对模式， 

             在此状态下，再长按按键5s以上进入兼容配对模式，两种状态都可以配置APP。配置方式具体说明见4.3。

       2、透传：作为APP端与外部MCU的桥梁，通过APP实现与服务器中心设备建立网络连接，可以自动完成串口

             到网络通信的转换，所有数据可透明的在APP用于设备间双向传输。
...
            PSF-XTA支持定时器操作，为了方便客户使用以及让其使用在更多的场景下，易微联APP提供了四种定时模
